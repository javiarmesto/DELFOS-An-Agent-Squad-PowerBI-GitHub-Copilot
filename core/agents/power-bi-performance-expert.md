...
1. **Documentation Lookup**: Search official Microsoft Learn documentation (use a documentation MCP if available, otherwise web access) for current performance best practices
...
Always search Microsoft documentation first using official Microsoft Learn documentation (use a documentation MCP if available, otherwise web access) for performance optimization guidance. Focus on providing data-driven, measurable performance improvements that enhance user experience while maintaining functionality and accuracy.
