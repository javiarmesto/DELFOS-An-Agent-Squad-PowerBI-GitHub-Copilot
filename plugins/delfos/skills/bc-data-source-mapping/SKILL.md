...
Treat the bundled API catalog as a discovery aid, not proof of availability. Before marking an entity or field COVERED, verify the target environment metadata and current Microsoft API documentation. Record tenant/environment/company and evidence; otherwise mark coverage UNVERIFIED. Never infer an exposed API from a BC table name. Check the installed ALDC specification contract before claiming compatibility; an untested handoff is a draft.
...
> Source: docs/plans/{project}/architecture.md
...
- Spec file: `docs/plans/{project}/{project}-bc-api.spec.md`
- Feed to ALDC: `the installed ALDC specification workflow` with the spec file
...
Architecture: docs/plans/order-tracker/architecture.md
...
- [ALDC skill-api](https://github.com/javiarmesto/ALDC-AL-Development-Collection) — API Page implementation patterns
- Delfos Architect — Star schema design source
